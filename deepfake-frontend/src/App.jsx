    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      ...
      <div className="card p-6 md:p-8 bg-[#0f172a]/50 backdrop-blur-md border border-white/10">
        ...
        <div className="flex items-center gap-5">
           {user.picture ? (
             <img src={user.picture} alt="Profile" className="w-16 h-16 rounded-full border-2 border-indigo-500 shadow-lg"/>
           ) : (
...
export default function App() {
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch("https://mobile-srm-deepfake-detector.onrender.com/predict", {
        method: "POST",
        body: formData,
      });
      ...
      <main className="flex-1 md:ml-[280px] p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            {user.picture ? (
               <img src={user.picture} alt="Profile" className="w-10 h-10 rounded-full border-2 border-indigo-500 shadow-lg object-cover"/>
            ) : (
